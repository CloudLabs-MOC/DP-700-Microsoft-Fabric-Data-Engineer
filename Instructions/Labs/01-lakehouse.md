...
1. On the **New item** page, search for **Lakehouse (2)** and select **Lakehouse (3)** from the result.  
...
1. Provide the following details to create a **Lakehouse**, and ensure to **uncheck the box (2)** before creating the Lakehouse. Then click on **Create (3)** to proceed.
   ...
   - **Name:** Enter **lakehouse<inject key="DeploymentID" enableCopy="false"/> (1)**  
     ...
     ![Screenshot of uploaded files in a lakehouse.](./Images/lakehouse1-p3.png)   
...
1. Name the subfolder as **data (3)** and then click **Create (4)**.

    ![Screenshot of uploaded files in a lakehouse.](./Images/subfolder-p3.png)
...
1. Then to upload the **sales.csv (1)** , click on **Upload (2)**  
   ...
   ![Screenshot of uploaded files in a lakehouse.](./Images/upload-p3.png) 
...
1. After the file has been uploaded, select the **Files/data** folder and verify that the **sales.csv** file has been uploaded, as shown here:
    ...
    ![Screenshot of uploaded files in a lakehouse.](./Images/upload1-p3.png) 

1. Select the **sales.csv** file to see a preview of its contents.

    ![Screenshot of uploaded sales.csv file in a lakehouse.](./Images/sales-p3.png)
...
2. View the available data source types for shortcuts. Then close the **New shortcut** dialog box without creating a shortcut.
    ...
    ![Screenshot of uploaded sales.csv file in a lakehouse.](./Images/sales1-p3.png)
...
2. In the **ellipsis (...)** menu for the **sales.csv** file, select **Load to Tables (1)** > **New table (2)**.

    ![Screenshot of uploaded sales.csv file in a lakehouse.](./Images/loadtotables-p3.png)

3. In **Load file to a new table** dialog box, set the table name to **sales (1)** and confirm the **Load (2)** operation. Then wait for the table to be created and loaded.

    ![Screenshot of uploaded sales.csv file in a lakehouse.](./Images/loadtotables1-p3.png)

    > **Tip**: If the **sales** table does not automatically appear, in the **ellipsis (...)** menu for the **Tables** folder, select **Refresh**.

3. In the **Lakehouse explorer** pane, select the **sales** table that has been created to view the data.

    ![Screenshot of a table preview.](./Images/loadtotables2-p3.png)

4. In the **ellipsis (...) (1)** menu for the **sales** table, select **View files (2)** to see the underlying files for this table
...
1. At the top-right of the Lakehouse page, switch from **Lakehouse (1)** to **SQL analytics endpoint (2)**. Then wait a short time until the SQL analytics endpoint for your lakehouse opens in a visual interface from which you can query its tables.
    ...
    ![Screenshot of a table preview.](./Images/sqlanalytics-p3.png)

1. Use the **New SQL query** button to open a new query editor.

    ![Screenshot of a table preview.](./Images/newsqlquery-p3.png)
...
1. Enter the following SQL query **(1)** use the **&#9655; Run (2)** button to run the query and view the **Results (3)**, which should show the total revenue for each product.
   ...
   ORDER BY Revenue DESC;
    ...
    ![Screenshot of a table preview.](./Images/newsqlquery1-p3.png)
...
1. On the toolbar, expand the **New SQL query (1)** option and select **New visual query (2)**.
    ...
    ![Screenshot of a table preview.](./Images/newvisualquery-p3.png)
...
2. Expand `dbo>Tables`, drag the **sales** table to the new visual query editor pane that opens to create a Power Query as shown here: 
    ...
    ![Screenshot of a Visual query.](./Images/newvisualquery1-p3.png)
...
3. In the **Manage columns (1)** menu, select **Choose columns (2)**. Then select only the **SalesOrderNumber (3)** and **SalesOrderLineNumber (4)** columns and then click **OK (5)**.
    ...
    ![Screenshot of a Choose columns dialog box.](./Images/column-p3.png)

    ![Screenshot of a Choose columns dialog box.](./Images/column1-p3.png)

4. In the **Transform (1)** menu, select **Group by (2)**.

    ![Screenshot of a Choose columns dialog box.](./Images/dpp61.png)

5. Then group the data by using the following **Basic** settings, then click **OK (5)**.
    ...
    - **Column**: SalesOrderLineNumber **(4)**
      ...
      ![Screenshot of a Choose columns dialog box.](./Images/group-p3.png)    
...
![Start Your Azure Journey](./Images/nextpage-03.png)
