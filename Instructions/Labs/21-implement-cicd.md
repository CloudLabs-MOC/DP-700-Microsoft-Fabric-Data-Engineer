...
1. Select **Deployment pipelines (2)**, then click on **New pipeline (3)**.
...
1. In the **Add a new deployment pipeline** window, enter **pipeline<inject key="DeploymentID" enableCopy="false"/> (1)** as the pipeline name, then click **Next (2)** to continue.
   ...
   ![Screenshot of pipeline stages.](./Images/lab5u18.png)
    ...
    >**Note:** If prompted, accept the defaults on the **Customize your stages** window.
...
1. From the top, select the **pipeline<inject key="DeploymentID" enableCopy="false"/> (1)** you created, then toggle off **New Deployment pipelines** to disable the feature **(2)**.
    ...
    ![Screenshot of a new lakehouse in Fabric.](./Images/mod5-p5t4p1.png)
